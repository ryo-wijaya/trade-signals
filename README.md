...
- **Action alert** — the only automatic push alert (`app.scheduler.run_action_alert_check`, checked every `priority_interval_minutes`, default 30, configurable via `/priority`, Mon–Fri during market hours). Originally two separate alerts (technical-only "Priority Alert", valuation-only "Cheap Alert") but they proved spammy on their own, so they were merged into ONE alert that requires **all 5** of the following before it fires — deliberately rare, meant to read as "this is a definite buy right now": (1) valuation score reads cheap/very cheap; (2) mean-reversion trigger score ≥ `scheduler.priority_min_signals` (default 2 of 3) on the **buy side only**; (3) that side's confirmation gate passes (confirmed bounce — same gate `/signals` uses, so never a falling knife); (4) revenue AND earnings growth both above `scheduler.action_alert_min_growth` (default 0.0); (5) analyst consensus in `scheduler.action_alert_analyst_labels` (default `["buy", "strong_buy"]`). Only once all 5 pass does it spend one live AI call (`build_prompt`, detailed mode) as a final confirmation — if the AI's own verdict isn't `BUY`, the alert is **vetoed** even though everything else passed. No API key set → the AI leg is skipped (treated as passed), not blocking. A failed/empty AI call leaves the ticker **unresolved** (retried next cycle) rather than silently vetoing or falsely confirming. Edge-triggered like the alerts it replaced — fires once per qualifying streak, must drop out and requalify to fire again. Labeled `ACTION ALERT:`, includes the AI's reasoning under the usual technical/valuation/target block.
...
The action alert uses in-memory dedup state (`_action_resolved` in `app/scheduler.py`) that resets on every redeploy/restart — after a restart, a currently-qualifying ticker is treated as freshly qualifying and gets a fresh AI check. Same accepted tradeoff as the rest of the app's in-memory state (sessions, LEAPS alert dedup) for a single-user personal tool with no database.

The action alert's interval is configurable at runtime via `/priority` (kept its original command name). The morning report time is fixed (`scheduler.morning_report_hour`/`morning_report_minute` in `config.json`) since there's no meaningful "interval" for a once-a-day report — an earlier version tried to expose a batch-report interval via `/interval`, but since the app moved to daily bars that interval was silently ignored by the scheduler (it always fired once at market close regardless of what you set). `/interval` has been removed rather than left dangling.

**Web UI** (`/ui`, password-gated): every command below can also be triggered from a browser, hosted on the same Fly machine as the bot — see "Web UI" further down for the full design.
...
WEB_UI_PASSWORD=a_password_only_you_know      # required to log into /ui; unset means the web UI always rejects login (fails closed, never open)
...
fly secrets set \
  ...
  WEB_UI_PASSWORD=a_password_only_you_know \
  OPENROUTER_API_KEY=your_openrouter_key
```

Generate a random `TRADE_SIGNALS_API_KEY` with: `openssl rand -hex 32`. `WEB_UI_PASSWORD` should be something you can actually type/remember, not a random hex string — it's a human password for `/ui`, not a machine API key.
...
| `/priority` | View or change action alert check frequency |
...
`https://your-app-url/ui` — a password-gated web page that can trigger any registered bot command (`app/web_ui.py`), hosted on the exact same Fly machine/process as the bot and REST API — no separate service. Log in once with `WEB_UI_PASSWORD` (a session cookie keeps you logged in for 30 days); every registered command shows up as a row with an optional args text box (space-separated, exactly like typing them after the command in Telegram — e.g. `NVDA CRM` or `fav`), auto-uppercased the same way the Telegram bot does. Submitting a row **runs the real command** — same code path, same data, same effects (including watchlist/favourites/priority changes) as triggering it from Telegram.

**Where results show up depends on how a command was triggered**, not on the command itself: trigger from Telegram and the result posts to Telegram, exactly as before; trigger from `/ui` and the result renders directly on the page instead. This works via `app/telegram.py:collect_output()` — a context manager around a `contextvars.ContextVar` that `send()` checks first: if a "collector" is active for the current async context, the message is appended to it instead of posted to Telegram; otherwise `send()` behaves exactly as it always has. No command handler needed to change for this — the compute/formatting layer was already Telegram-agnostic (every scan/report function returns a plain string), the coupling was narrowly that handlers called `send()` directly as their only output path. The web route sets a collector, `await`s the command to completion, and renders whatever accumulated — unlike the Telegram bot's fire-and-forget dispatch, so a slow command (e.g. `/deepdive` across several tickers) will make the page wait rather than returning instantly.

**Security notes:**
- The password is checked with a constant-time comparison (`secrets.compare_digest`) and **fails closed** — if `WEB_UI_PASSWORD` isn't set, login always rejects, it never falls open (unlike `TRADE_SIGNALS_API_KEY`'s REST API, which is deliberately open when unset for local dev).
- Failed logins get a 1-second delay — a simple brute-force throttle, no IP tracking or lockout state.
- Sessions are an in-memory token → expiry map inside the single process — they don't survive a redeploy or restart, which is a fine tradeoff for a single-user tool with no database.
- The session cookie is `HttpOnly`, `Secure`, and `SameSite=Strict` — `Secure` requires real HTTPS (Fly.io terminates TLS in front of the app), so if you test locally over plain `http://localhost`, the browser will silently refuse to send the cookie back; use `fastapi dev` with a tool like `mkcert`/`ngrok` for local HTTPS, or temporarily relax the flag for local-only testing.
- Every command is exposed, including state-changing ones (`/add`, `/remove`, `/fav`, `/unfav`, `/priority`) — there's no extra confirmation step beyond being logged in, by design (this is a personal single-user tool).
...
### PE Quality — is the reported P/E real or distorted by a one-off?

A trailing P/E is only as good as the earnings behind it, and GAAP net income can include large non-operating items that have nothing to do with recurring business performance — the textbook case is a big investment mark-to-market gain (ASU 2016-01 requires unrealized gains/losses on equity stakes to flow through the income statement; confirmed live: GOOGL's Q1 2026 GAAP net income included a ~$37B gain, largely from its stakes in companies like SpaceX and Anthropic appreciating). A large legal settlement, asset write-off, or divestiture shows up the same way, just in the other direction.

`app/valuation.py:_pe_quality` catches this from yfinance's *quarterly* income statement: sum the last 4 quarters' `Net Income` (GAAP) and `Normalized Income` (Yahoo's own ex-unusual-items figure), divide each by the latest quarter's diluted share count, and compare — both computed the *same self-consistent way*, deliberately not mixed with yfinance's separately-sourced headline `trailingPE`/`trailingEps`, which uses its own black-box TTM window and can disagree by a wide margin (confirmed live: GOOGL's headline `trailingEps` of 19.94 didn't match a plain sum of its last 4 quarters' `Diluted EPS` of ~13.1 — comparing core EPS against that number instead of a self-consistent GAAP-TTM figure would've produced a misleading gap).

- **"inflated"**: GAAP net income was boosted by a gain beyond `valuation.pe_quality_distortion_threshold` (default 15%) of TTM net income — the reported P/E looks *cheaper* than recurring earnings power supports (confirmed live: GOOGL read ~26% inflated, core P/E ~33 vs a self-computed GAAP-TTM P/E of ~24).
- **"suppressed"**: a one-off charge pushed GAAP net income down — the reported P/E looks *richer* than it should (confirmed live: PFE read ~106% suppressed by a litigation-style charge, core P/E ~9 vs GAAP-TTM P/E ~19).
- **"normal"**: distortion under threshold (confirmed live: NVDA, MSFT, META all sat under 2%) — no row shown.
- **"unknown"**: not enough quarterly `Normalized Income` data to judge (e.g. ETFs, some smaller/foreign filers) — no row shown; never a false "normal" claim.

Shown as a `PE Quality` row right under `P/E` in every signals/signalsplus/deepdive block, **only when it actually matters** — silently omitted otherwise, so it doesn't add noise to the ~95% of tickers with no meaningful distortion. Fed into the same `_valuation_line` used by the `/signalsplus` and `/deepdive` AI prompts, with an explicit instruction to weigh it (temper conviction, name the type of item for `/deepdive` if the data indicates which) but never restate the specific core P/E number or percentage, since it's already shown in this row — same "shown as data, not AI prose" pattern as the `Target` row.

### Valuation ranking (`/cheap`)

`/cheap` (watchlist), `/cheap fav` (favourites), `/cheap TICKER...` (`app/commands/cheap.py`) — ranks **every ticker in scope**, cheapest to most expensive, by the 0-100 score above. Shows a compact table (score, ticker, band) plus **every computable valuation number per ticker** (`_valuation_detail`) rather than picking just the single most-extreme signal — trailing P/E vs its own historical range (plus forward P/E vs that range when notably cheaper/richer), PEG vs the 1.0/2.0 cheap/expensive lines, and P/S vs its own historical range, all together, e.g. *"Trailing P/E 19.7 is below its entire 4yr range (27.1–786.3), and the forward P/E of 10.5 is cheaper still. PEG 0.57 — paying only 0.57x the earnings growth rate, under the 1.0 undervalued line. P/S 20.3 is below its 4yr average..."*, followed by a deterministic *"Analyst target: $303 (+43% vs price) · 58 analysts, strong buy"* line (`format_target`) when Yahoo has coverage. Tickers with zero computable signal (ETFs) are listed separately as "insufficient financial history" — **never silently dropped**, so the row count in the table plus that footer should always add up to the number of tickers you asked about.

The table/number block itself stays deterministic (no AI) — same data always produces the same ranking. When run interactively with `OPENROUTER_API_KEY` set, one AI message is appended (`build_cheap_stock_prompt`/`build_cheap_portfolio_prompt` in `app/llm.py`, `llm.cheap_stock_max_tokens`/`llm.cheap_portfolio_max_tokens`): for a **single** scored ticker, a 4-6 sentence "Why It's Priced This Way" paragraph weighing the valuation numbers, growth/margin, the analyst price target's upside/downside, and the overall technical rating together, explicitly barred from inventing a different cheap/fair/expensive label than the one already computed and required to flag anything that cuts against that verdict rather than giving a one-sided pitch; for **multiple** scored tickers, a 5-7 sentence "Portfolio Take" synthesis (not a per-ticker recap) naming which names are genuinely cheap with a supportive picture, which look cheap on paper but carry a warning sign, and the single most attractive vs. most-worth-avoiding name. Neither AI mode is allowed to restate a target's dollar figure or upside/downside percentage in its own words — that number is already shown in the deterministic "Analyst target" line above it, so the prompts only let it shape the reasoning qualitatively. The morning report auto-appends a filtered, **AI-free** version (`only_cheap=True`, titled "Cheap Right Now") showing only the very-cheap/cheap bands, to keep daily noise and AI cost low; run `/cheap` directly any time for the full ranking including fair/expensive names plus the AI take.
